...
Run `furrow row status [name]` for compact state, then read ONLY these three sources:
...
3. **Current step skill**: use the current step from `furrow row status` and load the matching rendered skill.
