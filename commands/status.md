...
1. Find task via `furrow row status [name]` or focused unit resolution (above).
...
Run `furrow row list --active` to get every active (non-archived) row in a compact table. The focused row is marked with `*`.
